...
typedef struct pathfinder_path {
  heap_node_t *hn;
  uint8_t pos[2];
  int32_t cost;
} pathfinder_path_t;
...
#define distancepair(pair) (d->distance[pair[dim_y]][pair[dim_x]])
#define distancexy(x, y) (d->distance[y][x])
#define tunnelpair(pair) (d->tunnel[pair[dim_y]][pair[dim_x]])
#define tunnelxy(x, y) (d->tunnel[y][x])
...
static void dijkstra_pathfinder(dungeon_t *d)
{
  ...
  heap_init(&h, pathfinder_path_cmp, NULL);
  ...
  //for loop to update distance map
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      distancexy(x,y)=path[y][x].cost;
...
static void dijkstra_pathfinder_tunneling(dungeon_t *d)
{
  static pathfinder_path_t path[DUNGEON_Y][DUNGEON_X], *p;
  static uint32_t initialized = 0;
  heap_t h;
...
void render_distance(dungeon_t *d)
{
  for(int y = 0;y<DUNGEON_Y;y++){
    for(int x = 0;x<DUNGEON_X;x++){
      if(d->pc[dim_x] == x && d->pc[dim_y] == y){
        putchar('@');
      }
      else if(distancexy(x,y)==255){
        printf(" ");
      }else{
        printf("%d",distancexy(x,y)%10);
      }
    }
    printf("\n");
  }
...
void render_tunnel(dungeon_t *d)
{
  for(int y = 0;y<DUNGEON_Y;y++){
    for(int x = 0;x<DUNGEON_X;x++){
      if(d->pc[dim_x] == x && d->pc[dim_y] == y){
      ...
      else if(tunnelxy(x,y)==255){
        printf(" ");
      }else{
        printf("%d",tunnelxy(x,y)%10);
      }
    }
    printf("\n");
  }
