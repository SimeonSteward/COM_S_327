...
void update_seen(dungeon_t *d){
  int y,x;
  for(y = -PC_VISION;y<=PC_VISION;y++){
    for(x = -PC_VISION;x<=PC_VISION;x++){
      int abs_x = d->pc.position[dim_x]+x;
      int abs_y = d->pc.position[dim_y]+y;
      if(abs_x>0 && abs_x<DUNGEON_X && abs_y>0 && abs_y<DUNGEON_Y){
        seenxy(abs_x,abs_y) =
          mapxy(abs_x,abs_y);
      } 
    }
...
void io_display(dungeon_t *d)
{
  ...
  for (y = 0; y < 21; y++) {
    for (x = 0; x < 80; x++) {
      if (d->character[y][x]&&(!(d->fog_of_war)||
                               (abs(y-d->pc.position[dim_y])<=PC_VISION&&
                                abs(x-d->pc.position[dim_x])<=PC_VISION))
          ) {
...
uint32_t io_teleport_pc(dungeon_t *d,pair_t dest)
{
  ...
  if(d->character[dest[dim_y]][dest[dim_x]]!=NULL){
    do_combat(d,&d->pc,charpair(dest));
      }
...
void io_teleport_handle_input(dungeon_t *d){
  uint32_t fail_code;
  int key;
  int x,y;
  pair_t dest;
  ...
  do{
    ...
    x = std::max(x,1);
    x = std::min(x,DUNGEON_X - 2);
    y = std::max(y,1);
    y = std::min(y,DUNGEON_Y - 2);
  } while (fail_code);
}
void io_handle_input(dungeon_t *d)
{
  ...
  do {
    ...
    case 'L':
      ...
      break;
    case 'g':
      /* Teleport the PC to a selected point in the dungeon*/
      io_teleport_handle_input(d);
      fail_code = 0;
      break;
    case 'm':
      io_list_monsters(d);
