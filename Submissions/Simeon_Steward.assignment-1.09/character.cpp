...
uint32_t character_get_speed(const character *c,const dungeon* d)
{
  if(c!=d->PC){
    return c->speed;
  }
    ...
    int ret_val = c->speed;
    if(PC->weapon_slot){
      ret_val += PC->weapon_slot->get_speed();
    }
    if(PC->offhand_slot){
      ret_val += PC->offhand_slot->get_speed();
    }
    if(PC->ranged_slot){
      ret_val += PC->ranged_slot->get_speed();
    }
    if(PC->armor_slot){
      ret_val += PC->armor_slot->get_speed();
    }
    if(PC->helmet_slot){
      ret_val += PC->helmet_slot->get_speed();
    }
    if(PC->cloak_slot){
      ret_val += PC->cloak_slot->get_speed();
    }
    if(PC->gloves_slot){
      ret_val += PC->gloves_slot->get_speed();
    }
    if(PC->boots_slot){
      ret_val += PC->boots_slot->get_speed();
    }
    if(PC->amulet_slot){
      ret_val += PC->amulet_slot->get_speed();
    }
    if(PC->light_slot){
      ret_val += PC->light_slot->get_speed();
    }
    if(PC->ring_one_slot){
      ret_val += PC->ring_one_slot->get_speed();
    }
    if(PC->ring_two_slot){
      ret_val += PC->ring_two_slot->get_speed();
    }
    return ret_val;
  }
}
