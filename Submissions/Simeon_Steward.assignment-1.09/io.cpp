...
static void io_list_wear(dungeon *d){
  for(size_t i = 0;i<21;i++){
    mvprintw(i,0,"                                                                          ");
  }
  for(size_t i = 0;i<PC_CAPACITY;i++){
    if(i<d->PC->carry.size()){
      mvprintw(i+6,9,"%d) %s (sp: %d, dmg: %d+%dd%d)",i,d->PC->carry[i]->get_name(),d->PC->carry[i]->get_speed(),
               d->PC->carry[i]->get_damage_base(),d->PC->carry[i]->get_damage_number(),
               d->PC->carry[i]->get_damage_sides());
    }else{
      mvprintw(i+6,9,"%d)",i);
    }
  }
  mvprintw(18,9,"To wear press the number of what you want to wear");

  pc_wear(d,getch()-48);
  io_display(d);
}

static void io_list_drop(dungeon *d){
  for(size_t i = 0;i<21;i++){
    mvprintw(i,0,"                                                                          ");
  }
  for(size_t i = 0;i<PC_CAPACITY;i++){
  ...
  mvprintw(18,9,"To drop press the number of what you want to drop");
  pc_drop(d,getch()-48);

  io_display(d);
}
static void io_inspect(dungeon *d){
  for(size_t i = 0;i<21;i++){
    mvprintw(i,0,"                                                                          ");
  }
  for(size_t i = 0;i<PC_CAPACITY;i++){
  ...
  mvprintw(18,9,"To inspect press the number of what you want to inspect");
  int choice = getch()-48;
  for(size_t i = 0;i<21;i++){
    mvprintw(i,2,"                                                       ");
  }
  if(choice>=0&&choice<(int)d->PC->carry.size()){
    mvprintw(6,2,"%s",d->PC->carry[choice]->get_description());
  }

  mvprintw(18,9,"press any key to continue                               ");
  getch();
  io_display(d);
}
...
static void io_list_inventory(dungeon *d){
  ...
  mvprintw(18,9,"Inventory, press any key to return to game");
  getch();
  io_display(d);
}
...
static void io_list_equipment(dungeon *d){
  ...
  mvprintw(6,5, "Weapon:                                           ");
  mvprintw(7,5, "Offhand:                                          ");
  mvprintw(8,5, "Ranged:                                           ");
  mvprintw(9,5, "Armor:                                            ");
  mvprintw(10,5,"Helmet:                                           ");
  mvprintw(11,5,"Cloak:                                            ");
  mvprintw(12,5,"Gloves:                                           ");
  mvprintw(13,5,"Boots:                                            ");
  mvprintw(14,5,"Amulet:                                           ");
  mvprintw(15,5,"Light:                                            ");
  mvprintw(16,5,"Ring 1:                                           ");
  mvprintw(17,5,"Ring 2:                                           ");

  if(d->PC->weapon_slot){
    object &obj = *(d->PC->weapon_slot);
    mvprintw(6,14,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->offhand_slot){
    object &obj = *(d->PC->offhand_slot);
    mvprintw(7,14,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->ranged_slot){
    object &obj = *(d->PC->ranged_slot);
    mvprintw(8,14,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->armor_slot){
    object &obj = *(d->PC->armor_slot);
    mvprintw(9,14,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->helmet_slot){
    object &obj = *(d->PC->helmet_slot);
    mvprintw(10,14,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->cloak_slot){
    object &obj = *(d->PC->cloak_slot);
    mvprintw(11,14,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->gloves_slot){
    object &obj = *(d->PC->gloves_slot);
    mvprintw(12,14,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->boots_slot){
    object &obj = *(d->PC->boots_slot);
    mvprintw(13,14,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->amulet_slot){
    object &obj = *(d->PC->amulet_slot);
    mvprintw(14,14,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->light_slot){
    object &obj = *(d->PC->light_slot);
    mvprintw(15,14,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->ring_one_slot){
    object &obj = *(d->PC->ring_one_slot);
    mvprintw(16,14,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->ring_two_slot){
    object &obj = *(d->PC->ring_two_slot);
    mvprintw(17,14,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
...
static void io_list_take_off(dungeon *d){
  mvprintw(6,5, "Weapon(a):                                           ");
  mvprintw(7,5, "Offhand(b):                                          ");
  mvprintw(8,5, "Ranged(c):                                           ");
  mvprintw(9,5, "Armor(d):                                            ");
  mvprintw(10,5,"Helmet(e):                                           ");
  mvprintw(11,5,"Cloak(f):                                            ");
  mvprintw(12,5,"Gloves(g):                                           ");
  mvprintw(13,5,"Boots(h):                                            ");
  mvprintw(14,5,"Amulet(i):                                           ");
  mvprintw(15,5,"Light(j):                                            ");
  mvprintw(16,5,"Ring 1(k):                                           ");
  mvprintw(17,5,"Ring 2(l):                                           ");
  mvprintw(18,9,"To take off press the letter of what you want to take off");
  if(d->PC->weapon_slot){
    object &obj = *(d->PC->weapon_slot);
    mvprintw(6,18,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->offhand_slot){
    object &obj = *(d->PC->offhand_slot);
    mvprintw(7,18,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->ranged_slot){
    object &obj = *(d->PC->ranged_slot);
    mvprintw(8,18,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->armor_slot){
    object &obj = *(d->PC->armor_slot);
    mvprintw(9,18,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->helmet_slot){
    object &obj = *(d->PC->helmet_slot);
    mvprintw(10,18,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->cloak_slot){
    object &obj = *(d->PC->cloak_slot);
    mvprintw(11,18,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->gloves_slot){
    object &obj = *(d->PC->gloves_slot);
    mvprintw(12,18,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->boots_slot){
    object &obj = *(d->PC->boots_slot);
    mvprintw(13,18,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->amulet_slot){
    object &obj = *(d->PC->amulet_slot);
    mvprintw(14,18,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->light_slot){
    object &obj = *(d->PC->light_slot);
    mvprintw(15,18,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->ring_one_slot){
    object &obj = *(d->PC->ring_one_slot);
    mvprintw(16,18,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }
  if(d->PC->ring_two_slot){
    object &obj = *(d->PC->ring_two_slot);
    mvprintw(17,18,"%s (sp: %d, dmg: %d+%dd%d)",obj.get_name(),obj.get_speed(),obj.get_damage_base(),
             obj.get_damage_number(),obj.get_damage_sides());
  }

  pc_take_off(d,getch()-97);
  io_display(d);
}

static void io_expunge(dungeon *d){
  for(size_t i = 0;i<PC_CAPACITY;i++){
    mvprintw(i+6,9,"                                              ");
  }
  for(size_t i = 0;i<PC_CAPACITY;i++){
  ...
  mvprintw(18,9,"To expunge press the number of what you want to expunge");
  pc_expunge(d,getch()-48);

  io_display(d);
}

static void io_look_monster(dungeon *d){
  pair_t dest;
  int c;
  fd_set readfs;
  struct timeval tv;
  ...
  mvprintw(0, 0,
           "Choose a character on the map. press 't' on the character or 'esc' to abort ");
  ...
  * not otherwise used.                                                 */
  ...
  if(c == 't'){
    if(charpair(dest) && charpair(dest) != d->PC){
      ...
      mvprintw(1,12,"%s (sp: %d, dmg: %d+%dd%d)",(charpair(dest))->name,(charpair(dest))->speed,
               (charpair(dest))->damage->get_base(),
               (charpair(dest))->damage->get_number(),(charpair(dest))->damage->get_sides());
      mvprintw(3,2,"%s",((npc*)charpair(dest))->description);
      mvprintw(18,9,"press any key to continue                               ");
      getch();
    }else{
